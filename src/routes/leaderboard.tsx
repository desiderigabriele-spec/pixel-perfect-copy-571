...
function LeaderboardPage() {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => fetchLb({ data: { scope, period, filterValue: filterValue || undefined, special, limit: 20 } }),
    initialData: scope === "global" && period === "all" && !filterValue && !special ? initial : undefined,
    staleTime: 30_000,
  });
  const items = data?.items ?? [];

  const filterOptions = scope === "language"
    ? [["it","Italiano"],["en","English"],["es","Español"],["de","Deutsch"],["fr","Français"]]
    : scope === "country"
    ? [["IT","🇮🇹 Italia"],["US","🇺🇸 USA"],["GB","🇬🇧 UK"],["DE","🇩🇪 Germania"],["FR","🇫🇷 Francia"],["ES","🇪🇸 Spagna"]]
    : scope === "style"
    ? [["scalper","Scalper"],["intraday","Intraday"],["swing","Swing"]]
    : scope === "asset"
        ...
        <div className="space-y-3">
          <div className="flex gap-2 flex-wrap">
            {(["global","language","country","style","asset"] as Scope[]).map((s) => (
              <button
                key={s}
                onClick={() => { setScope(s); setFilterValue(""); setSpecial(undefined); }}
            ...
            <button
              onClick={() => { setScope("global"); setFilterValue(""); setSpecial("rising_star"); }}
              className={`px-3 py-1.5 border font-mono text-[10px] uppercase tracking-widest ${
                special === "rising_star"
