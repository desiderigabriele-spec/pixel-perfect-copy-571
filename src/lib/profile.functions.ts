...
export const getLeaderboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    let profQ = supabaseAdmin
      .from("profiles")
      .select(
        "id, username, avatar_seed, points_balance, country, language, style, primary_asset, created_at",
      )
      .limit(1000);
    if (data.scope === "language" && data.filterValue)
      profQ = profQ.eq("language", data.filterValue);
    if (data.scope === "country" && data.filterValue) profQ = profQ.eq("country", data.filterValue);
    if (data.scope === "style" && data.filterValue)
      profQ = profQ.eq("style", data.filterValue as any);
    if (data.scope === "asset" && data.filterValue)
      profQ = profQ.eq("primary_asset", data.filterValue);
...
export const getCurrentLiveMatch = createServerFn({ method: "GET" }).handler(async () => {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { data: c } = await supabaseAdmin
    .from("challenges")
    .select(
      "id, symbol, duration_minutes, creator_id, opponent_id, creator_side, opponent_side, starts_at, ends_at, stake_type, stake_amount, visibility",
    )
    .eq("status", "live")
...
export const getPublicLiveMatch = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: c } = await supabaseAdmin
      .from("challenges")
      .select(
        "id, symbol, duration_minutes, creator_id, opponent_id, creator_side, opponent_side, starts_at, ends_at, status, visibility",
      )
      .eq("id", data.id)
      .eq("visibility", "public")
      .maybeSingle();
